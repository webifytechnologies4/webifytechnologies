...
class Media {
  ...
  createShader() {
    ...
    img.src = this.image;
    img.onload = () => {
      // SVGs loaded from CDN might have 0 or undefined for naturalWidth/naturalHeight in WebGL
...
class App {
  constructor(container, { items, bend, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05, autoScroll = false, autoScrollSpeed = 0.5 } = {}) {
  ...
  update() {
    // Auto-scroll: continuously move target forward when user is not interacting
    if (this.autoScroll && !this.userInteracting) {
